## $Id: ar-fp.R,v 1.1 2023/08/17 17:23:09 zchi Exp zchi $
...
sample.fp  <- function(n, alpha, alpha0=2/3, theta0=6*pi/7, Delta=.5)
### Sampling routine in section 7.3
{
    z0=1; z1=(1-alpha)*10^-30; alpha1=max(alpha0, 0.9);

    z = rep(0,n); y=rep(0,n); log.y = rep(0,n); theta = rep(0,n);

    delta=1-alpha;
    ## Sample _z_ 
    I = 1:n;
    while(length(I)>0) {
        z[I] = sample.z(length(I), alpha);
        I = which(z<=0);
    }

    ## Work on entries in _z_ that are >= _z0_
    I = which(z>=z0);
    if (length(I)>0) {
    ...
    if (length(I)>0) {
        ## If _alpha_<= _alpha1_, work on entries in _z_ that are < _z0_.
        ## If _alpha_>_alpha1_, work on entries in _z_ in [_z1_, _z0_]. 
    ...
    if (alpha>alpha1 & length(I)>0) {
        ## If _alpha_ > _alpha1_, work on entries in _z_ that are < _z1_
    ...
    return(list(z=z, y=y, log.y=log.y, theta=theta));
}
