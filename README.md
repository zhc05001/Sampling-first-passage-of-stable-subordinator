...
- **`z`:** the value of a random variable denoted $`z`$.  It is the first random variable to be sampled by `sample.fp`.  Given the value of $`z`$, the time $`\tau`$ of the first passage is a determined by $`\tau = B^{-1}(s)`$, where $`s = \alpha[(1-\alpha)/z]^{1/\alpha-1}`$.
